...
int match(char * pattern, char * text){
	...
	if(pattern_i >= pattern_len) break; //找到匹配字符(其实j只会==pattern_len)
	}
...
2.KMP算法:查询表
(1)构造查询表next[0,pattern_len):在任一位置pattern[pattern_i]处失败后,将j替换为next[pattern_i]
...
int match(char * pattern, char * text){
	...
	while( pattern_i < pattern_len && text_i < text_len ){ //自左向右,逐个比对字符
		if( pattern_i < 0 || text[text_i] == pattern[pattern_i] ){ //若匹配
			text_i++; pattern_i++;
