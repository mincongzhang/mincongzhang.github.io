...
private:
  ...
  StrPointIslandMap m_island_map;
  int m_num_island;
  int m_count;
...
/////////////////////////////////////////////////////////
  ...
  int search(const int x, const int y){
    StrPointIslandMap::iterator search_it;

    search_it = m_island_map.find(strHash(x+1,y));
    ...
    search_it = m_island_map.find(strHash(x,y+1));
    if(search_it != m_island_map.end())
        return search_it->second;
...
public:
  vector<int> numIslands2(int n, int m, vector<Point>& operators) {
    ...
    m_count = 0;
    //Point p1 = operators[0];
    //std::cout<<strHash(-1,p1.y)<<std::endl;
    //std::cout<<strHash(p1)<<std::endl;
    
    for(vector<Point>::const_iterator it = operators.begin(); it != operators.end(); ++it){
        Point p = *it;
        std::string p_hash = strHash(p);
        StrPointIslandMap::iterator map_it = m_island_map.find(p_hash);
        if(map_it == m_island_map.end()){ //not found
            
            int island_id = search(p.x,p.y);
        ...
        m_count_vec.push_back(m_count);
    }
  
  
    //TODO: merge, get smaller id, m_count--  
    return m_count_vec;
  }
};
```
